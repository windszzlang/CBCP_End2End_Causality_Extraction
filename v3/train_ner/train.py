...
def train(epochs=20, print_freq=100, batch_size=16, lr=2e-5, device='cpu', max_len=512,
    ...
    named_entity = ['CAUSESUB', 'CAUSEPRED', 'EFFECTSUB', 'EFFECTPRED']
    labels = ['O']
    ...
    for epoch in range(1, epochs + 1):
        ...
        with torch.no_grad():
            ...
            for d in train_data:
                ...
                res = recognize(d[0], d[2], tokenizer, model, id2label, device)
                for i, word in enumerate(res):
                    pred_entitis[i].append(word)
                res = label_sequence_to_entity(d[0], d[1], isLabel=True)
                for i, word in enumerate(res):
                    gold_entitis[i].append(word)
...
if __name__ == '__main__':
    ...
    train(
        ...
        device='cuda',
        max_len=512,
        data_path='./data/ner_quadruple/',
        tokenizer_path='./models/pt/chinese_roberta_wwm_ext_large',
        pretrained_model_path='./models/pt/chinese_roberta_wwm_ext_large',
        resume=False,
        load_path='./models/saved/xxx.pt',
        save_path='./models/saved/CBC_large_best.pt',
        multi_gpu=False
    )
